...
class AC_Network():
    ...
    def train(self,sess,gamma):
        ...
        while not coord.should_stop():
            ...
            result,l,abs_errors,_ = sess.run([self.merged_summary,self.loss, self.u, self.apply_grads_global],feed_dict=feed_dict)
            ...
            while not coord.should_stop():
                ...
                while not d:
                    ...
                    if GLOBAL_STEP > 500000 and GLOBAL_STEP % 1000 and worker_num == 33:
                        ...
                        for i in range(2):
                            Index_list_L = sorted(random.sample(range(int(beg + i * 4), int(beg + (i + 1) * 4)), 3 - i))
                    ...
                    if GLOBAL_STEP > 500000 and GLOBAL_STEP % 1500 and worker_num == 33 and ALL_best > the_rEward_dic[0][1]:
                        # range(num_workers/2 - 8,num_workers/2 + 8)
                        beg = num_workers / 2 - 8
                        end = num_workers / 2 + 8
                    ...
                    if len_of_relay % batch_size ==0 and len_of_relay != 0 and d != True and worker_num == 33:
                        sess.run(self.update_ops[int(self.local_AC.target_scope.split('_')[1])][int(the_rEward_dic[0][0].split('_')[1])])

                        if GLOBAL_STEP <= 500000 or self.unpermit:
                            sess.run(self.update_local_ops)
                        else:
                            for i in range(2):
                                Index_list_from = sorted(random.sample(range(int(i * 4), int((i + 1) * 4)), 4 - i))
                                Index_list_to = sorted(
                                    random.sample(range(int(num_workers - (i + 1) * 4), int(num_workers - i * 4)), 4 - i))
                                ...
                                for j in range(4 - i):
                                    ...
                                    if random.random() * (num_workers + 1) <= 1:
                                        update_pg_op = \
                                        self.update_part_ops[int(the_rEward_dic[int(Index_list_from[j])][0].split('_')[1])][
                                            int(the_rEward_dic[int(Index_list_to[j])][0].split('_')[1])]
                                    else:
                                        update_pg_op = \
                                        self.update_part_ops[int(the_rEward_dic[int(Index_list_from[j])][0].split('_')[1])][
                                            int(the_rEward_dic[int(Index_list_to[j])][0].split('_')[1])]
                                    sess.run(update_pg_op)
                                    rEward_dic[the_rEward_dic[int(Index_list_to[j])][0]] = (rEward_dic[the_rEward_dic[int(Index_list_from[j])][0]]+rEward_dic[the_rEward_dic[int(Index_list_to[j])][0]])/2
                                    self.unpermit = True
                ...
                if len(self.episode_rewards)>5:
                    rEward_dic[self.name] = np.mean(self.episode_rewards[-5:])
                    the_rEward_dic = sorted(rEward_dic.iteritems(), key=operator.itemgetter(1), reverse=True)
                    if self.local_AC.i!=0 and self.local_AC.i%500:
                        t_idx = -1
                        for t_id in range(len(the_rEward_dic)):
                            if the_rEward_dic[t_id][0] == self.local_AC.scope:
                                t_idx = t_id
                        if t_idx == -1:
                            print('something wrong')
                            sys.exit(0)
                        self.local_AC.target_scope = 'worker_'+str(the_rEward_dic[t_idx//4][0].split('_')[1])
                ...
                if np.mean(self.episode_rewards[-7:]) >= ALL_best and np.var(self.episode_rewards[-7:]) < 2 and worker_num == 33:
