            ...
            while not coord.should_stop():
                ...
                while not d:
                    ...
                    if len_of_relay % batch_size == 0 and len_of_relay != 0 and d != True and worker_num == 33:
                        sess.run(self.update_ops[int(self.local_AC.target_scope.split('_')[1])][
                                     int(the_rEward_dic[0][0].split('_')[1])])
                        if self.local_AC.target_scope == "worker_" + str(num_workers):
                            rEward_dic[the_rEward_dic[0][0]] = ALL_best
                        else:
                            rEward_dic[the_rEward_dic[0][0]] = rEward_dic[self.local_AC.target_scope]
                        ...
                        if GLOBAL_STEP <= 500000 or self.unpermit:
                            ...
                            for i in range(2):
                                ...
                                Index_list_to = sorted(
                                    random.sample(range(int(num_workers - (i + 1) * 4), int(num_workers - i * 4)),
                                                  4 - i))
                                ...
                                for j in range(4 - i):
                                    ...
                                    if random.random() * (num_workers + 1) <= 1:
                                        update_pg_op = \
                                            self.update_part_ops[int(self.local_AC.target_scope.split('_')[1])][
                                        ...
                                        update_pg_op = \
                                            self.update_part_ops[
                                                int(the_rEward_dic[int(Index_list_from[j])][0].split('_')[1])][
                                                int(the_rEward_dic[int(Index_list_to[j])][0].split('_')[1])]
